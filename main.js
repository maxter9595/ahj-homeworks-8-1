...
class ChatApp {
  ...
  bindEvents() {
    document.getElementById("continueButton").onclick = () => this.registerUser();
    document.getElementById("messageInput").onkeypress = e => this.handleMessageInput(e);
  }
  registerUser() {
  ...
  appendMessage(data) {
    ...
    if (data.nickname === this.nickname) {
      ...
      messageElement.innerHTML = `
              <div class="meta">You, ${new Date().toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit"
      ...
      messageElement.innerHTML = `
              <div class="meta">${data.nickname}, ${new Date().toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit"
      })} ${new Date().toLocaleDateString()}</div>
              <div>${data.message}</div>
  ...
  setupCustomScrollbar() {
    ...
    const updateScrollThumb = () => {
      ...
      const thumbHeight = Math.max(chatBoxVisibleHeight / chatBoxHeight * scrollBarHeight, 20);
      const thumbTop = chatBox.scrollTop / (chatBoxHeight - chatBoxVisibleHeight) * (scrollBarHeight - thumbHeight);
